...
function multiPlayer(){
    ...
    cards.forEach(card => {
        ...
        card.addEventListener("click", () => {
            ...
            correct.push(number-1)
            if(correct.length==num)
            {
                order=correct
                gamePlay()
            }
        ...
        card.addEventListener("click", () => {
            ...
            if(answer.indexOf(s)==-1 && answerKey.indexOf(s)!=-1) //checks if block is correct and if it isnt repeating
            {
                ...
                if(answer.indexOf(s)==answerKey.indexOf(s)){      //checks order
                    ...
                    if(answer.length==num)                         //if final round
                    {
                        if(player==0)
                        {
                        score+=10
                        ...
                        else if(player==2)
                        {
                            score+=10;
                            let score2=score;
                            console.log(score2 + localStorage.getItem("player1Score"))
                            console.log()
                    ...
                    else if(player==1)
                    {
                        ...
                        localStorage.clear();
                        location.reload()
                        alert("Player 2 has made an error. Enter 2 for player 2 to set the stage")
                    ...
                    else
                    {
                        ...
                        localStorage.clear();
                        location.reload()
                        alert("player 1 has made an error. Click ok to find winner")
                        if(localStorage.getItem("player1Score") > score2)
                        {
                            localStorage.clear();
                            alert("player 1 wins")
